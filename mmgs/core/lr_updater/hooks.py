...
class HoodLrUpdaterHook(LrUpdaterHook):
    ...
    def __init__(self, 
                 ...
                 decay_steps: int = 5e6,
                 step_start: int = 0,
                #  decay_steps: Union[int, list[int]] = (5e6, [5e6]),
                #  step_start: Union[int, list[int]] = (0, [0]),
                 **kwargs) -> None:
    ...
    def _sched_fun(self, step):
        # if isinstance(self.decay_steps, list):
        #     assert isinstance(self.step_start, list) and len(self.decay_steps) == len(self.step_start)
        #     cum_decay_exp = 0
        #     for i in range(len(self.decay_steps)):
        #         if i+1 >= len(self.decay_steps):
        #             # Exceed the len
        #             s_start = self.step_start[i]
        #             d_steps = self.decay_steps[i]
        #             break
        #         elif self.step_start[i+1] > step and self.step_start[i] <= step:
        #             # Within the range
        #             s_start = self.step_start[i]
        #             d_steps = self.decay_steps[i]
        #             break
        ...
        #                 cum_decay_exp += 0
        #             else:
        #                 cum_decay_exp += (self.step_start[i] - self.step_start[i-1]) // self.decay_steps[i-1]
        #     cur_step = max(0, step - s_start)
        #     decay = self.decay_rate ** (cur_step // d_steps + cum_decay_exp) + 1e-2
        # else:
