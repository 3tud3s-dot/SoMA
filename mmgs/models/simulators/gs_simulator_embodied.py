...
class GsSimulatorEmbodied(BaseSimulator):
    ...
    def __init__(self,
                ...
                #  neck=None,
                 ...
                 force_forward=-1,
                 **kwargs):
        super(GsSimulatorEmbodied, self).__init__(init_cfg)
    ...
    def init_weights(self):
        super(GsSimulatorEmbodied, self).init_weights()
    ...
    def save_gaussian(self, scene_name, frame_idx, pred_pos, pred_cov):
        ...
        save_dir = os.path.join(self.data_dir, scene_name, 'pred_stage1')
    ...
    def simple_test(self,
        ...
        """Simple test with single image.
            ...
            pred_frame_idx: source frame index t; this method predicts t + 1.
        ...
        if self.flag_save_gaussian and pred_frame_idx == 0:
            self.save_gaussian(scene_name, 0, original_mov_state, original_mov_cov)
        num_frame = min(gt_label[0].shape[1] - pred_frame_idx - 1, rollout_size) if gt_label is not None else rollout_size

        rst_dict = defaultdict(list)
        for frame_idx_ in range(num_frame):
            frame_idx = frame_idx_ + pred_frame_idx
            ...
            if self.flag_save_gaussian:
                self.save_gaussian(scene_name, cur_label_idx, pred_pos[num_controller_points:], pred_cov[num_controller_points:])
    ...
    def train(self, mode=True):
        ...
        super(GsSimulatorEmbodied, self).train(mode)
        if not self.opt_sim:
