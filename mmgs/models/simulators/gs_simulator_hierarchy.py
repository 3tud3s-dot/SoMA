...
class GsSimulatorHierarchy(BaseSimulator):
    ...
    def encode_decode(self, input_graph, connect_graph, gaussians, cam_list, scene_cov3D, scene_pcs, mov_mask, register_norm=False, gt_label=None, rollout_size=1, cln_mask=None, cln_gaussian=None, white_bg_list=None, opacity_scalar=None):
        ...
        return pred_pos, pred_cov, outF_lv_list, skeleton_dict, img_list, losses
    ...
    def forward_train(self, inputs, gt_label, num_epoch=0, num_iter=0, **kwargs):
        ...
        N = original_state.shape[0]
        ## n_total points
        mov_mask = torch.ones((N,), dtype=torch.bool, device=device) #inputs['mov_mask'].squeeze(0)
        ## n_moving pionts
        pin_mask = torch.zeros((N, 1), dtype=torch.float32, device=device) #inputs['pin_mask'].squeeze(0)
        pin_mask[0] = 1
        ## cluster mappings
        # p2c_mapping_list = [torch.ones((N, 1), dtype=torch.bool) for p2c in inputs['p2c_mapping']] # [p2c.squeeze(0) for p2c in inputs['p2c_mapping']]
        p2c_object_list = [p2c[1].squeeze(0).to(torch.int64) for p2c in inputs['p2c_mapping']]
        p2c_controller_list = [p2c[0].squeeze(0).to(torch.int64) for p2c in inputs['p2c_mapping']]
        p2c_mapping_list = []
        for p2c_object, p2c_controller in zip(p2c_object_list, p2c_controller_list):
            # ids = max(p2c_object)
            ids = min(p2c_object)
            p2c_mapping = p2c_object - ids
            p2c_mapping_list.append(p2c_mapping)
        p2c_mapping_list = p2c_mapping_list[0:-1]

        
        ## For render acceleration
        cln_mask = torch.ones((N,), dtype=torch.bool, device=device) #inputs['cln_mask'].squeeze(0)
        ...
        original_cov = scene_gaussian.get_covariance() # TODO: here is not correct
        original_mov_cov = original_cov[mov_mask, ...]
        # Get scene init_pos, lazy initialize
        ...
        #TODO; how to initialize the self.scene_initn1_pos and scene_init0_pos?
        ## n_point, 3
        ...
        if static_pred:
            # num_frame += 1 # Extra frame to predict static at the beginning # Or just keep the total num of frame unchanged
        ...
        for frame_idx in range(num_frame):
            if static_pred:
                if frame_idx == 0:
                    ...
                    prev_state = cur_state # Which is original_mov_state
                elif frame_idx == 1:
                    # Input: t'0, t-1; Output: t1
                    prev_state = tn1_state
                    # Input: t0, t-1; Output: t1. This one force the input to be correct
                    cur_state = original_mov_state
                    if self.pred_vel:
                        cur_state = t0_state
            else:
                if frame_idx == 0:
                    ...
                    if self.pred_vel:
                        cur_state = t0_state
                # Frame_idx > 0, auto-regressive
            ## according to p2c_mapping build input_graph(basic graph) and connect_graph(with hierarchical structure)
            ...
            # TODO
            pred_pos, pred_cov, pred_dg_list, knowledge_dict, pred_img_list, losses_i = self.encode_decode(
                ...
                mov_mask, gt_label=cur_label, register_norm=True, # TODO: only opt the sim?
            ...
            if self.selfsup_loss:
                if not self.selfsup_velonly or (static_pred and frame_idx == 1 or not static_pred and frame_idx == 0):
                    selfsup_loss = self._encode_decode_train_selfsup(pred_pos, cur_state, prev_state, input_graph[0], pred_dg_list, num_iter=num_iter if not self.train_cfg['by_epoch'] else num_epoch)
                    losses_i.update(selfsup_loss)
    ...
    def teacher_distill(self, 
        ...
        # Init frame structure: prev_state+cov, cur_state+cov; rest state+cov;
        ## n_point, attr_dim
        attr = self.scene_attr[scene_name]
        diag_volume = torch.prod(original_mov_scaling*inputs['volume_scalar'].squeeze(0), dim=-1, keepdim=True)
        ...
        if pred_frame_idx is None or pred_frame_idx == 1:
            # First is for validating; second for testing
        ...
        if zero_init:
            cur_state = original_mov_state
            prev_state = cur_state
        # cur_cov is always the one from rest states
        ...
        for frame_idx in range(num_frame):
            ...
            # knowledge_dict['F'].append(pred_dg_list)
            # knowledge_dict['DG'].append(out_dg_list)
    ...
    def simple_test(self,
        ...
        for frame_idx in range(num_frame):
            ...
            # Forward: predicting the velocity or positions
            cur_label_idx = pred_frame_idx if pred_frame_idx is not None else frame_idx+1
            cur_label = [gt[0, min(cur_label_idx, num_frame-1)] for gt in gt_label] # Currently all have gt_label
            pred_pos, pred_cov, pred_dg_list,knowledge_dict,  pred_img_list, losses_i = self.encode_decode(
                input_graph, connect_graph,
                scene_gaussian, inputs['cam'],
                original_cov.detach().clone(), original_state.detach().clone(), # For rendering, better not change this two variable
