...
class TimeResidualFFN(BaseModule):
    """Implements feed-forward networks (FFNs) with residual connection.
    Args:
        embed_dims (list): The feature dimensions.
        act_cfg (dict, optional): The activation config for FFNs.
    ...
    def __init__(self,
                ...
                #  norm_cfg=dict(type='LN'),
                 ...
                 bias=True,
                 init_cfg=None,):
        super(TimeResidualFFN, self).__init__(init_cfg)
        ...
        self.layer2 = nn.Sequential(
            Linear(out_channels, out_channels, bias=bias), self.activate,
            nn.Dropout(dropout))
            
        self.norm1 = build_norm_layer(norm_cfg, in_channels)[1] if norm_cfg is not None else nn.Identity()
        self.norm2 = build_norm_layer(norm_cfg, out_channels)[1] if norm_cfg is not None else nn.Identity()

        self.time_emb = Linear(time_channels, out_channels, bias=bias)
        

    def forward(self, x, residual=None, t=None, **kwargs):
        """Forward function for `FFN`."""
        assert t is not None
        # h = self.layer1(self.activate(self.norm1(x)))
        out = self.layer1(self.norm1(x))
        t_emb = self.time_emb(t)
        # out += t_emb[:, None] # Here relu will cannot backwards
        out = self.layer2(self.norm2(out + t_emb[:, None]))
