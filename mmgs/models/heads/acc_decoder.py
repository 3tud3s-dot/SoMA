...
class AccDecoder(SimHead):
    ...
    def pre_render(self, cam, gt_label, white_bg, scene_gaussian, cov3D_precomp, pos, opacity, rotation=None, return_tuple=False, **kwargs):
        # Conver sh first
        # opacity = scene_gaussian.get_opacity
        ...
        if white_bg is not None:
            if white_bg:
                ...
                # render_bg = 85/255 * torch.ones_like(render_bg, dtype=torch.float32).to(pos.device) # For CVPR grey bg
            else:
    ...
    def forward_train_static(self, pred_pos, gt_pos, **kwargs):
        # Only for static input and static output
        losses = dict()
        # The max_val for  ssim is 1 for default
    ...
    def forward_train(self,
        ...
        losses = dict()
        # The max_val for  ssim is 1 for default
        controller_img_mask = kwargs.pop('controller_img_mask', None)
        bbox = kwargs.pop('bbox', None)
        mask_weights_list = None
        if controller_img_mask is not None:
            mask_weights_list = [1. - mask.permute(1, 2, 0) for mask in controller_img_mask]
        # loss_render = self.loss(
        #     [pi.permute(1,2,0) for pi in pred_img], [gt.permute(1,2,0) for gt in gt_label],
        #     term_filter=['render'], **kwargs)
        if bbox is not None:
            pred_img = [pi[:, bbox[i, 0]:bbox[i, 2], bbox[i, 1]:bbox[i, 3]] for i, pi in enumerate(pred_img)]
            gt_label = [gt[:, bbox[i, 0]:bbox[i, 2], bbox[i, 1]:bbox[i, 3]] for i, gt in enumerate(gt_label)]

            mask_weights_list = [mi[bbox[i, 0]:bbox[i, 2], bbox[i, 1]:bbox[i, 3], :] for i, mi in enumerate(mask_weights_list)]
        loss_render_masked =  self.loss(
            [pi.permute(1,2,0) for pi in pred_img], [gt.permute(1,2,0) for gt in gt_label],
            term_filter=['render'], mask_weights=mask_weights_list, **kwargs)
        losses.update(loss_render_masked)

        return losses
    
    def forward_train_embodied(self, pred_img, gt_label, **kwargs):
        losses = dict()
        #spatial loss
        loss_spatial = self.loss(
            [pi.permute(1,2,0) for pi in pred_img], [gt.permute(1,2,0) for gt in gt_label],
            term_filter=['spatial'], **kwargs)
        losses.update(loss_spatial)

        #bounding loss
        loss_bounding = self.loss(
            [pi.permute(1,2,0) for pi in pred_img], [gt.permute(1,2,0) for gt in gt_label],
            term_filter=['weight'], **kwargs)
        losses.update(loss_bounding)

        return losses

    def forward_train_motion(self, student_dict, teacher_dict, **kwargs):
        losses = dict()
        for key in student_dict.keys():
            pred_value = student_dict[key]
            gt_value = teacher_dict[key]
            loss_motion = self.loss(
                pred_value, gt_value,
                term_filter=[key],**kwargs)
            losses.update(loss_motion)

        return losses

    def forward_train_selfsup(self, pred_pos, cur_state, prev_state, base_graph, dg_list, num_iter=None):
        losses = dict()

        density = self.density_activation(base_graph.ndata['attr'][:, 0:1])
        mass = density * base_graph.ndata['diag_volume']
        external_forces = base_graph.ndata['external']
        # Dynamics
        kinetic = 0.5 * mass * torch.sum(((pred_pos - 2*cur_state + prev_state)/self.dt)**2, dim=-1, keepdim=True)
        external = torch.sum(external_forces*pred_pos, dim=-1, keepdim=True)

        # Inertia
        total_inertia = torch.sum(kinetic, dim=0, keepdim=True)
        loss_inertia = self.loss(
            total_inertia, torch.zeros_like(total_inertia).to(total_inertia),
            term_filter=['inertia'],
            num_iter=num_iter,
        )
        losses.update(loss_inertia)
        # Gravity
        total_gravity = torch.sum(external, dim=0, keepdim=True)
        loss_gravity = self.loss(
            total_gravity, torch.zeros_like(total_gravity).to(total_gravity),
            term_filter=['gravity'],
            num_iter=num_iter,
        )
        ...
        total_potential = 0
        for dg in dg_list:
            cx = torch.matmul(dg.transpose(-1, -2), dg) - torch.eye(dg.shape[-1]).to(dg)
            cx2 = cx ** 2
            c_trace = cx.diagonal(offset=0, dim1=-1, dim2=-2).sum(dim=-1, keepdim=True) + cx2.diagonal(offset=0, dim1=-1, dim2=-2).sum(dim=-1, keepdim=True)
            total_potential = total_potential + c_trace.sum(dim=0, keepdim=True)
        loss_potential = self.loss(
            total_potential, torch.zeros_like(total_potential).to(total_potential),
            term_filter=['potential'],
            num_iter=num_iter,
        )
        losses.update(loss_potential)
    ...
    def node_pred_pos_dg(self, feature_field, anchor_next_state_field, anchor_template_field, template_field, pin_mask_field, pos_field, deformation_gradient_field, deformation_gradient_matrix_field, apply_pin):
        def func(nodes):
            ...
            pred_deformation_gradient = self.dynamic_proj(feature) #FFN
            ...
            eps = 1e-7
            normed_U = pred_U / (torch.linalg.norm(pred_U, dim=-1, keepdim=True) + eps)
            ...
            normed_V = pred_V / (torch.linalg.norm(pred_V, dim=-1, keepdim=True) + eps)
            pred_deformation_gradient = torch.cat([normed_U, normed_Scalar, normed_V], dim=-1) #* self.ratio
            # if torch.any(torch.isnan(pred_deformation_gradient)):
            #     assert not torch.any(torch.isnan(pred_deformation_gradient))
            ...
            Ft = DeformationGradient.get_deformation_gradient_matrix(pred_deformation_gradient) * self.ratio
            next_pos = anchor_next_state + torch.bmm(Ft, (template-anchor_template).unsqueeze(-1)).squeeze(-1)
            ## Mask out
            if pin_mask_field in nodes.data.keys():
