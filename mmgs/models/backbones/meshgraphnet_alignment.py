...
class MeshGraphNetAlign(BaseBackbone):
    ...
    def __init__(self,
                 ...
                 fix_bug=False,
                 **kwargs):
        super(MeshGraphNetAlign, self).__init__()
        ...
        # States embeddings
        ## vel
        node_dim = position_dim+attr_dim if attr_mode == 'cat' else position_dim # Only for input the encoder
    ...
    def init_edge_features_fixbug(self, cur_state_field, orig_template_state_field, anchor_next_state_field, anchor_orig_template_state_field, edge_out_field):
        def func(edges):
            # n_edges, 3
            recv_state = edges.dst[cur_state_field]
            send_state = edges.src[cur_state_field]
            ...
            if self.edge_mode == 'ordinary':
                # n_edges, (3+1)*2
                in_emb = self.edge_normalizer(torch.cat([delta_pos, norm_delta_pos, delta_orig_tem, norm_delta_orig_tem], dim=-1))
            else:
                assert self.edge_mode == 'ratio'
                # n_edges, (3+1)
                ## direction, length magnitude
                in_emb = self.edge_normalizer(torch.cat([delta_pos/norm_delta_pos, norm_delta_pos/norm_delta_orig_tem], dim=-1))
            if self.edge_theta:
                # theta in template state
    ...
    def init_node_features(self, anchor_cur_state_field, anchor_next_state_field, cur_state_field, 
                           attr_field, node_out_field):
        def func(nodes):
            # Static info
            ...
            anchor_vel = (anchor_next_state - anchor_cur_state) / (self.dt)

            cur_state = nodes.data[cur_state_field]
            vel = (cur_state-anchor_next_state)
    ...
    def init_features(self, graph,
                      anchor_cur_state_field, anchor_next_state_field, cur_state_field,
                      template_state_field, anchor_template_state_field,
                      orig_template_state_field, anchor_orig_template_state_field,
                      attr_field, pin_mask_field,
                      node_out_field, edge_out_field):
        
        # Apply to all nodes
        graph.apply_nodes(self.init_node_features(
            anchor_cur_state_field, anchor_next_state_field,
            cur_state_field,
            attr_field, node_out_field))
        # Apply to all edges
        if self.fix_bug:
            graph.apply_edges(self.init_edge_features_fixbug(
                cur_state_field, orig_template_state_field, anchor_next_state_field, anchor_orig_template_state_field, edge_out_field))
        else:
    ...
    def forward(self, graph, connect_graph, **kwargs):
        ...
        graph = self.init_features(
            graph,
            'anchor_cur_state', 'anchor_next_state',
            'cur_state',
            'template_state', 'anchor_template_state',
            'orig_template_state', 'anchor_orig_template_state',
            'attr', 'pin_mask',
