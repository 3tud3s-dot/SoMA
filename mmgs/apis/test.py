...
from mmgs.datasets.utils import writePKL, to_numpy_detach, writeJSON, readPKL
...
def single_gpu_rollout_interact(model, data_loader, show=False, out_dir=None, **show_kwargs):
    ...
    force_template_seqidx = []
    force_template_data_ = []
    ...
    for i, data in enumerate(data_loader):
        ...
        if scene_name == 'pudding':
            ...
            # ]
        elif scene_name == 'mothorchids_v3':
            k = 4
            type='custom'
            query_idx = 8470
            radius = 5.0
            decay_radius = 0.75
        elif scene_name == 'carnation':
            k = 1
            type = 'custom'
            query_idx = 22230
            radius = 5.0
            ...
            raise NotImplementedError(f"Scene {scene_name} not implemented")
        

        force_template_data = [
                dict(type=type, query_idx=query_idx, radius=radius, decay_radius=decay_radius, data=pright_vec*k),
                dict(type=type, query_idx=query_idx, radius=radius, decay_radius=decay_radius, data=pup_vec*k),
                dict(type=type, query_idx=query_idx, radius=radius, decay_radius=decay_radius, data=pleftup_vec*k),
            ]
        ...
        for frame_idx in range(interact_range):
            ...
            if frame_idx+1 in inject_frame_idx_list and frame_idx not in inject_frame_idx_list:
                ...
                elif force_data['type'] == 'drag':
                    cur_state = dataset.inject_external_forces_drag(cur_state, cur_state[force_data['query_idx']].reshape(1,3), force_data['radius'], force_data['decay_radius'], force_data['data'], pin_mask)
                else:
                    pass
                force_pointer += 1
            with torch.no_grad():
...
def single_gpu_rollout(model, data_loader, show=False, out_dir=None, **show_kwargs):
    ...
    for i, data in enumerate(data_loader):
        ...
        frame_num = data['gt_label'][0].shape[1] - 1 # Temporarily use this; 
        # frame_num = data['meta']['seq_num'].item()
        controller_img_mask_list = data['inputs']['controller_img_mask_list']
        # controller_img_mask_list = data['inputs']['pure_robot_img_mask_list']
        # mask_object_list = data['inputs']['mask_object_list']

        full_video_list = data['inputs']['full_gt_label']
        masked_video_list = data['gt_label']
        # masked_video_list = mask_object_list
        bbox_raw = data['inputs']['bbox'].squeeze(0)
        # controller_mask_img_list = data['inputs']['controller_mask_img_list']
        assert frame_num > 0 # N frames provide N-1 supervised transitions.
        if eval_start_frame >= frame_num or eval_start_frame < 0:
            eval_start_frame = frame_num - 1
        
        if scene_name_cur is None or scene_name_cur != scene_name:
            print(f"Resetting state for scene {scene_name}")
            prev_state = None
            cur_state = None
            cur_cov = None
            scene_name_cur = scene_name
        ...
        # for frame_idx in range(3):
            with torch.no_grad():
                result = model(
                    return_loss=False, **data,
                    prev_state=prev_state, cur_state=cur_state, cur_cov=cur_cov, pred_frame_idx=frame_idx)
            ...
            if show or out_dir:
                # Can use model.show_result() to show the image result
                rendered_img = result['pred_img_list']
                if 'cur_img_list' in result.keys():
                    cur_render_img = result['cur_img_list']
                else:
                    cur_render_img = None
                ...
                cam = data['inputs']['cam']
                bbox_list = bbox_raw[:, frame_idx+1, :]
                save_image(rendered_img, cam, full_video_list, controller_img_mask_list, seq_num, scene_name, frame_idx, out_dir, bbox_list)
                # if frame_idx == 0:
                #     bbox_list = bbox_raw[:, frame_idx, :]
                #     save_image(cur_render_img, cam, full_video_list, masked_video_list, seq_num, scene_name, frame_idx-1, out_dir, bbox_list)
...
def save_image(rendered_img, cam, full_video_list, controller_img_mask_list, seq_num, scene_name, frame_idx, out_dir, bbox_list):
    # Save prediction, ground truth, and blended visualization.
    for img, cam, gt_img_list, controller_img_mask_lists, bbox in zip(rendered_img, cam, full_video_list, controller_img_mask_list, bbox_list):
        cam_path = cam.img_path
        # cam_id = os.path.splitext(os.path.basename(cam_path))[0].split('_')[-1]
        # frame_idx_ = (frame_idx + 1) * 10 -1

        cam_id = cam_path.split('/')[-2]
        seq_dir = osp.abspath(osp.join(out_dir, scene_name, f"seq_{str(seq_num.item()).zfill(5)}_cam_{cam_id}"))
        mmcv.mkdir_or_exist(seq_dir)
        

        controller_img_mask = np.array(controller_img_mask_lists[0][frame_idx+1])
        controller_img_mask = controller_img_mask.transpose(1, 2, 0)
        ...
        # blend image
        blended_save_path = os.path.join(seq_dir, f"blended_{str(frame_idx+1).zfill(5)}.png")
        alpha = 0.4
        bk_color = np.array([161, 157, 157]) / 255.0
        mask_gs = np.all(cv2_img <= 0.1, axis=2)#[:, :, None]
        cv2_img[mask_gs] = bk_color

        # blended_img = alpha * cv2_gt_img + (1-alpha) * cv2_img
        blended_img = cv2_gt_img * controller_img_mask + ((1-controller_img_mask) * (alpha * cv2_gt_img + (1-alpha) * cv2_img))
        blended_img = np.clip(blended_img, 0, 1.0)
        cv2.imwrite(
            blended_save_path,
            255 * blended_img,
        )
