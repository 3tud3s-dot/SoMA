...
from mmgs.datasets.pipelines import Compose
...
def inference_model(model):
    ...
    # Filter out the active mask, since active mask is to maskout from all kerenls. but we onlhy need the maskout from moving part points
...
def update_force(pin_mask, active_mask, force, cur_state=None, dt=1/30):
    ...
    if cur_state is not None:
        p_mask = pin_mask[0]
        # print(f"Activte mask: {active_mask};   force: {force};   dt: {dt}")
