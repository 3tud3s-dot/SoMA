...
def read_video_image_rgba_cv2_mask(video_dir, mask_dir, frame_idx_list, prompt_dict, white_bg=False, use_pil=False):
    """Read video using cv2, return [T, 3, H, W] array, fps"""
    cam_id = mask_dir.split('/')[-1]
    mask_info_json_name = f"mask_info_{cam_id}.json"
    mask_info_json_path = mask_dir.replace(f"/{cam_id}", f"/{mask_info_json_name}")
    mask_info_json = readJSON(mask_info_json_path)
    object_mask_ids = []
    robot_mask_ids = []
    obstacle_mask_ids = []
    
    for k, v in mask_info_json.items():
        # if v != 'hand':
        if v in prompt_dict['object']:
            object_mask_ids.append(k)
        if v in prompt_dict['obstacle']:
            obstacle_mask_ids.append(k)
            if 'robot' in v:
                robot_mask_ids.append(k)
    assert len(object_mask_ids) == 1, f"Object mask ids should be >=1, but got {len(object_mask_ids)}"
    object_mask_id = object_mask_ids[0]
        
    mask_ids_list = os.listdir(mask_dir)
    rst = defaultdict(list)
    for f_idx in frame_idx_list:
        # f_path = os.path.join(video_dir, f"{str(f_idx).zfill(5)}.png")
        img_name = f"{f_idx}.png"
        video_path = os.path.join(video_dir, img_name)
        mask_path_list = [os.path.join(mask_dir, mask_id, img_name) for mask_id in mask_ids_list]
        # mask_path_list = [os.path.join(mask_dir, object_mask_id, img_name)]
        
        
        # assert os.path.exists(video_path) or os.path.exists(video_path.replace(".png", ".jpg")), f"Path not exist: {video_path}"
        if not os.path.exists(video_path):
            video_path = video_path.replace(".png", ".jpg")
        assert os.path.exists(video_path), f"Path not exist: {video_path}"
        for mask_path in mask_path_list:
            if not os.path.exists(mask_path):
                mask_path = mask_path.replace(".png", ".jpg")
            assert os.path.exists(mask_path), f"Path not exist: {mask_path}"
        # H, W, 3
        if not use_pil:
            img = cv2.imread(video_path, cv2.IMREAD_UNCHANGED)
        else:
            with PIL.Image.open(video_path) as image:
                img = np.array(image.convert("RGBA"))
        assert img.dtype == np.uint8

        mask_list = {}
        for mask_ids, mask_path in zip(mask_ids_list, mask_path_list):
            if not use_pil:
                if not os.path.exists(mask_path):
                    mask_path = mask_path.replace(".png", ".jpg")
                mask = cv2.imread(mask_path, cv2.IMREAD_UNCHANGED)
            else:
                with PIL.Image.open(mask_path) as image:
                    mask = np.array(image.convert("RGBA"))
            img_cp = img.copy()
            real_mask = mask[..., np.newaxis] / 255.0
            img_cp = img_cp * real_mask #(mask[..., np.newaxis] / 255.0)
            ...
            if white_bg: #False
                # img_merge += np.ones_like(img_merge) * (1-norm_data[:, :, 3:4]) * 255.0 
                img_merge += np.ones_like(img_merge) * (1 - (mask[..., np.newaxis] / 255.0)) * 255.0
            img_bgr = np.clip(img_merge, 0, 255).astype(np.uint8)   
            img = np.clip(img, 0, 255).astype(np.uint8)
            # img_bgr = np.clip(img_merge, 0, 1).astype(np.uint8)
            
            if not use_pil: #False
                img_rgb = cv2.cvtColor(img_bgr, cv2.COLOR_BGR2RGB)
            else:
                img_rgb = img_bgr
            
            
            if mask_ids in object_mask_ids:
                rst['mask_object'].append(img_rgb)
            elif mask_ids in obstacle_mask_ids:
                if mask_ids in robot_mask_ids:
                    rst[f'mask_robot_{mask_ids}'].append(real_mask)
                rst[f"mask_{mask_ids}"].append(real_mask)
        if not use_pil:
            img = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)
        rst['image'].append(img)
        # rst.append(img_rgb)
    # T, H, W, 3
    # rst = np.stack(rst, axis=0)
    for mask_ids in rst.keys():
        rst[mask_ids] = np.stack(rst[mask_ids], axis=0)
    return rst