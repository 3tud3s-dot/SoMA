...
# from mmgs.core.evaluation import precision_recall_f1, support
# from mmcv.parallel import collate
# from .utils import ClothEnvReader, GARMENT_TYPE, COLLATE_GARMENT_TYPE
...
#             if n_steps > 1 and env_cfg.get("small_step", False):
#                 noise_small_range = np.linspace(0, noise_range[0], num=n_steps+1)[1:-1]
#                 noise_range = np.concatenate([noise_small_range, noise_range], axis=-1)
#             self.noise_range = noise_range
#         l_noise_std = env_cfg.get("l_noise_std", 0.0)
#         self.l_noise_range = None
#         if l_noise_std > 0:
#             l_n_steps = env_cfg.get("l_noise_step", 1)
#             l_noise_range = np.linspace(0, l_noise_std, num=l_n_steps+1)[1:]
#             if l_n_steps > 1 and env_cfg.get("small_step", False):
#                 l_noise_small_range = np.linspace(0, l_noise_range[0], num=l_n_steps+1)[1:-1]
#                 l_noise_range = np.concatenate([l_noise_small_range, l_noise_range], axis=-1)
#             self.l_noise_range = l_noise_range
#         self.obj_scalar = env_cfg.get("obj_scalar", 1.0)
    
#     def set_sequence(self, length):
#         self.non_grad_step = length
    
#     def _init_reader(self):
#         self.clothenv_reader = ClothEnvReader(self.env_cfg.clothenv_base, phase=self.phase)
...
#                 #  start_frame for static = dynamic + init_frame + history
#                 # Here history == 0, init_frame == 0
...
#                     eval_results[g_name] = []
#                 if g_rst > 0 or 'force' in g_name or 'potential' in g_name:
