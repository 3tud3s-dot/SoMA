...
def readQQTTSceneInfo(
    ...
    test_cam_infos = []
    # print(os.path.exists(os.path.join(path, "interp_poses.pkl")), os.path.join(path, "interp_poses.pkl"))
    if os.path.exists(os.path.join(path, "interp_poses.pkl")):
        test_c2ws = pickle.load(open(os.path.join(path, "interp_poses.pkl"), "rb"))
        for cam_i, c2w in enumerate(test_c2ws):
            dummy_cam_id = 1
            K = intrinsics[dummy_cam_id]
        ...
        for cam_i, c2w in enumerate(test_c2ws):
            ...
            print("No interp poses found, using test cameras from metadata.json")
            # test_cam_infos = [c for c in cam_infos if c.is_test]
