...
class GaussianModel:
    ...
    def create_from_pcd(self, pcd : BasicPointCloud, cam_infos : int, spatial_lr_scale : float):
        ...
        initial_color = torch.tensor([-1.7766138 , -1.2761885 , -1.0074382 ]).cuda()
        fused_point_cloud = torch.tensor(np.asarray(pcd.points)).float().cuda()
        # fused_color = RGB2SH(torch.tensor(np.asarray(pcd.colors)).float().cuda())
        fused_color = initial_color.repeat(fused_point_cloud.shape[0], 1)
        ...
        if self.isotropic:
            scales = torch.log(torch.sqrt(dist2)*1.5)[...,None]
            # scales = torch.sqrt(dist2)[...,None] * 1.2
        else:
            scales = torch.log(torch.sqrt(dist2)*1.5)[...,None].repeat(1, 3)
            # scales = torch.sqrt(dist2)[...,None].repeat(1, 3) * 1.2
