#!/usr/bin/env python3
"""T26.3 read-only forward-scale observer; no backward, updates, or fixes.
...
ap=argparse.ArgumentParser();ap.add_argument('--mode',choices=['d360','official','t22'],required=True);ap.add_argument('--out',type=Path,required=True);ap.add_argument('--fine',action='store_true');args=ap.parse_args()
...
def sanitize(x):
 ...
 return x
report=dict(task='T26.3',mode=args.mode,fine=args.fine,job=os.environ['SLURM_JOB_ID'],seed=5,tool_hash=sha(__file__),backward=False,optimizer_step=False,events=[],normalizers=[],passes=[],init_weights_calls=[])
  ...
  if isinstance(m,Normalizer):
   ...
   def post(mod,a,o):
    ...
    if args.fine:
     ...
     entry['variance_audit']=dict(actual_fp32_raw_variance=array(variance).tolist(),current_batch_centered_variance64=x.var(axis=0).tolist(),current_batch_mean64=x.mean(axis=0).tolist(),current_batch_centered_std64=x.std(axis=0).tolist(),max_centered_residual64=abs(x-x.mean(axis=0)).max(axis=0).tolist())
...
def run_iter(self,data_batch,train_mode,**kw):
 ...
 report['losses']=dict(self.outputs['log_vars']);raise Done()
...
def nf(*a):
 out=orig_native(*a);rr=array(out[2]);current.setdefault('rasterizer',[]).append(dict(camera_index=len(current.get('rasterizer',[])),positive_radius=int((rr>0).sum()),radius=dist(rr),positive_radius_dist=dist(rr[rr>0]),rendered_tile_instances=out[0]));return out
...
except Done:report['status']='OBSERVED_NO_BACKWARD'
