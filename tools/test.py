...
from mmgs.apis import multi_gpu_test, single_gpu_test
from mmgs.core import wrap_fp16_model
from mmgs.datasets import build_dataloader, build_dataset
from mmgs.models import build_simulator
from mmgs.utils import (auto_select_device, get_root_logger,
...
def parse_args():
    ...
    parser.add_argument('--max_seq', type=int, default=100, help='max sequence length')
    parser.add_argument('--work_dir', help='the dir to save logs and models')
    parser.add_argument('--interact', action='store_true', help='interact with the model')
    parser.add_argument(
        '--test-rollout-mode',
        choices=['segmented', 'continuous'],
        default=None,
        help='rollout mode used by stage2 simple_test')
...
def main():
    ...
    if args.max_seq < cfg.max_seq:
        k = int(cfg.data['train']['dataset']['env_cfg']['max_cam_total'] / cfg.data['train']['dataset']['env_cfg']['max_seq'])
        cfg.data['train']['dataset']['env_cfg']['max_cam_total'] = k * args.max_seq
        cfg.data['val']['env_cfg']['max_cam_total'] = k * args.max_seq
        cfg.data['test']['env_cfg']['max_cam_total'] = k * args.max_seq
        cfg.data['train']['dataset']['env_cfg']['max_seq'] = args.max_seq
        cfg.data['val']['env_cfg']['max_seq'] = args.max_seq
        cfg.data['test']['env_cfg']['max_seq'] = args.max_seq
        
    if args.cfg_options is not None:
        cfg.merge_from_dict(args.cfg_options)
    if args.test_rollout_mode is not None:
        cfg.model.test_rollout_mode = args.test_rollout_mode
    
    # set multi-process settings
    setup_multi_processes(cfg)
    cfg.data.test.env_cfg['interact'] = args.interact
    ...
    if 'anchor_points' in cfg.keys() and 'directed_edges' in cfg.keys():
        model.read_skeleton_info(cfg.anchor_points, cfg.directed_edges)
