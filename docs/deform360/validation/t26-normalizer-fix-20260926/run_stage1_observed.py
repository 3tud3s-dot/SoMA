...
CONTROL=ROOT/'outputs/deform360/t26-normalizer-fix-control-20260926'
WORK=ROOT/'outputs/deform360/stage1/config_a_seed5_normalizer_fix'
...
assert report['head']=='0a628fa4dec9d8d267b131187f06b9582bc18c7b'
...
preflight=json.loads((REPO/'docs/deform360/validation/t26-stage1-20260926/resumed_preflight.json').read_text());assert preflight['status']=='PASS'
assert preflight['config_sha256']==report['config_sha256']
assert all(sha(Path(p))==h for p,h in preflight['asset_hashes'].items())
report['normalizer_sha256']=sha(REPO/'mmgs/models/utils/normalization.py')
assert report['normalizer_sha256']=='f78712dd0760fcf16a58431d7fdfd240a2e38cba89c400b405df93798f364220'
report['work_dir']=str(WORK)
report['epoch_summaries']=[]
...
def run_iter(self,data_batch,train_mode,**kwargs):
 ...
 if not gaussian_before:
  ...
  normalizers=[m for m in model_ref.modules() if isinstance(m,Normalizer)]
  assert len(normalizers)==3
  for m in normalizers:
 ...
 if self.inner_iter==0:
  ...
  epoch_rows.clear()
 expected_lr=4e-4*(.5**(max(0,self.epoch-14)//2)+.01)
 assert all(abs(v-expected_lr)<1e-12 for v in self.current_lr())
...
def preprocess(self,*args,**kwargs):
 ...
 i=ctx['step']-1
 if i==0:
  initial_position=args[1].detach().clone();initial_covariance=args[8].detach().clone()
  previous_prediction=None
 ...
 assert i<15 and 123+10*i<268
 assert np.array_equal(args[3].detach().cpu().numpy(),expected_traj[i])
 assert np.array_equal(args[4].detach().cpu().numpy(),expected_traj[i+1])
...
def encode(self,*args,**kwargs):
 ...
 previous_prediction=result[0][30:].detach().clone()
...
def opt_hook(self,runner):
 ...
 for m in model_ref.modules():
  if isinstance(m,Normalizer):
   assert all(getattr(m,k).dtype==torch.float64 for k in ['_acc_count','_num_accumulations','_acc_sum','_acc_sum_squared'])
 ...
 report['last_completed_iteration']=row;save()
 return result
...
def checkpoint(self,*args,**kwargs):
 ...
 info={'path':str(p),'bytes':p.stat().st_size,'sha256':sha(p),'epoch':self.epoch+1,'iter':self.iter,'metadata_readable':True,'model_optimizer_present':True}
 del ck
 assert len(epoch_rows)==50
 ks=epoch_rows[0]['losses']
 epoch_summary={'epoch':self.epoch+1,'steps':len(epoch_rows),'last_iteration':self.iter,'rollout':min(3+3*self.epoch,15),'lr':self.current_lr(),'loss_mean':{k:float(np.mean([r['losses'][k] for r in epoch_rows])) for k in ks},'first_loss':epoch_rows[0]['losses'],'last_loss':epoch_rows[-1]['losses'],'preclip_norm_min':min(r['preclip_norm'] for r in epoch_rows),'preclip_norm_max':max(r['preclip_norm'] for r in epoch_rows),'postclip_norm_max':max(r['postclip_norm'] for r in epoch_rows),'all_gradient_finite':True,'training_memory':memory(),'checkpoint':info}
 report['epoch_summaries'].append(epoch_summary)
 report['last_good_checkpoint']=info
...
def evaluate(self,runner):
 ...
 guard(model_ref.state_dict(),'post-evaluation state')
 if report['epoch_summaries']:
  report['epoch_summaries'][-1]['evaluation_memory']=memory()
  report['epoch_summaries'][-1]['evaluation_metrics']=values
 event('evaluation_complete',metrics=values,memory=memory());save();return result
...
try:
 ...
 report['normalizer_unchanged']=sha(REPO/'mmgs/models/utils/normalization.py')==report['normalizer_sha256']
