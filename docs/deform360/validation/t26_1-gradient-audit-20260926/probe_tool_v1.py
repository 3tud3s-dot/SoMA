...
ap.add_argument('--anomaly',action='store_true');args=ap.parse_args()
...
def safe(x):
 ...
 return x
report={'task':'T26.1','tag':args.tag,'rollout':args.rollout,'selected_step':args.step,'component':args.component,'anomaly':args.anomaly,
...
def tap(t,name):
 if args.anomaly and torch.is_tensor(t) and t.requires_grad:
...
def render(self,*a,**kw):
 if args.anomaly:
  for key in ['pos','cov3D_precomp','opacity']:
   if key in kw:tap(kw[key],f'step{step}.cam{a[0].uid}.renderer_input.{key}')
 out=original_render(self,*a,**kw)
 if args.anomaly:tap(out[0],f'step{step}.cam{a[0].uid}.render_output')
 return out
